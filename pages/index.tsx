...
import { getCookie, setCookie, getJsonCookie, setJsonCookie } from "../lib/cookies";
...
export default function Home() {
  ...
  const fetchInbox = useCallback(async (addr?: string, silent = false) => {
    ...
    try {
      const r = await fetch(`/api/inbox?inbox=${encodeURIComponent(target)}`);
  ...
  const startSSE = useCallback((addr: string) => {
    ...
    const es = new EventSource(`/api/stream?inbox=${encodeURIComponent(addr)}`);
    esRef.current = es;
  ...
  const generateEmail = useCallback(async () => {
    ...
    try {
      const r = await fetch("/api/generator-email");
      const data = await r.json();
      if (data.auth?.email) {
        saveEmail(data.auth.email);
        setInbox([]);
        await fetchInbox(data.auth.email);
        startSSE(data.auth.email);
      }
    } catch {}
    setGenerating(false);
  }, [fetchInbox, startSSE]);
  ...
  const clearInbox = async () => {
    ...
    setClearing(true);
    try {
      await fetch(`/api/clear?inbox=${encodeURIComponent(emailRef.current)}`, { method: "DELETE" });
      setInbox([]);
    } catch {}
    setClearing(false);
  };
  ...
  const applyCustomEmail = async () => {
    ...
    try {
      const r = await fetch(`/api/custom-email?username=${encodeURIComponent(customUser.trim())}&domain=${encodeURIComponent(selectedDomain)}`);
      const data = await r.json();
      if (data.auth?.email) {
        saveEmail(data.auth.email);
        setInbox([]);
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Generate a free temporary email instantly. No registration. Real-time inbox with SSE streaming." />
      </Head>
          ...
          <div className="flex items-center justify-between px-4 sm:px-5 pt-4 pb-3 border-b border-[#21262d]">
            ...
            {recentAddresses.length > 1 && (
              <div className="relative">
                ...
                {showRecent && (
                  <div className="absolute right-0 top-6 z-30 bg-[#0d1117] border border-[#30363d] rounded-xl shadow-2xl py-1.5 min-w-60 animate-slide-up">
                    {recentAddresses.map((addr) => (
                      <button key={addr} onClick={() => {
                        setShowRecent(false);
                        if (addr === emailRef.current) return;
                        saveEmail(addr); setInbox([]); fetchInbox(addr); startSSE(addr);
                      }} className={`w-full text-left px-4 py-2.5 text-xs hover:bg-[#21262d] transition-colors font-mono truncate ${addr === emailRef.current ? "text-violet-400" : "text-[#c9d1d9]"}`}>
                        {addr}
                        {addr === emailRef.current && <span className="ml-2 text-[#484f58]">current</span>}
                      </button>
                    ))}
                  </div>
                )}
          ...
          <div className="px-4 sm:px-5 py-4">
            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center mb-4">
              ...
              <div className="flex gap-2 flex-shrink-0">
                ...
                <button onClick={() => { if (domains.length === 0) fetch("/api/domains").then(r => r.json()).then(d => { if (d.data?.domains) { const l = d.data.domains.filter((x:any)=>x.is_active).map((x:any)=>({domain:x.domain_name})); setDomains(l); if(l.length>0)setSelectedDomain(l[0].domain); } }); setShowCustomModal(true); }}
              ...
              {inbox.map((mail) => {
                ...
                return (
                  <button key={mail.id} onClick={async () => {
                    ...
                    markRead(mail.id);
                    try {
                      const r = await fetch(`/api/email/${mail.id}?inbox=${encodeURIComponent(emailRef.current)}`);
                      const data = await r.json();
                      ...
                      if (full && typeof full === "object") {
                        ...
                        if (merged.html_content) setHtmlView(true);
                      }
                    } catch {}
        ...
        <div className="fixed inset-0 z-[60] bg-black/75 backdrop-blur-sm flex items-end sm:items-center justify-center sm:p-4 animate-fade-in"
          ...
          <div className="bg-[#161b22] border border-[#30363d] w-full sm:rounded-2xl sm:max-w-3xl max-h-[92vh] sm:max-h-[90vh] flex flex-col shadow-2xl animate-slide-up rounded-t-2xl"
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#21262d] flex-shrink-0">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {modalLoading ? (
                  ...
                  <button onClick={() => setHtmlView(!htmlView)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${htmlView ? "bg-violet-600/20 border-violet-600/40 text-violet-300" : "bg-[#21262d] border-[#30363d] text-[#8b949e] hover:text-white"}`}>
                    {htmlView ? "Text" : "HTML"}
                  </button>
                ) : (
                  <span className="text-[10px] text-[#484f58] border border-[#30363d] px-2 py-1 rounded-lg">Text only</span>
                )}
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              ) : htmlView && selectedMail.html_content ? (
        ...
        <div className="fixed inset-0 z-[60] bg-black/75 backdrop-blur-sm flex items-end sm:items-center justify-center sm:p-4 animate-fade-in"
          ...
          <div className="bg-[#161b22] border border-[#30363d] w-full sm:rounded-2xl sm:max-w-sm flex flex-col shadow-2xl animate-slide-up rounded-t-2xl"
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#21262d]">
              ...
              <button onClick={() => setShowCustomModal(false)} className="w-8 h-8 rounded-xl bg-[#21262d] hover:bg-[#30363d] flex items-center justify-center transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-[11px] font-semibold text-[#8b949e] uppercase tracking-wider mb-2">Username</label>
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowCustomModal(false)} className="flex-1 py-3 rounded-xl bg-[#21262d] hover:bg-[#30363d] text-sm font-medium border border-[#30363d] transition-colors">
                  Cancel
                </button>
