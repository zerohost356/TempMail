...
export default function Domains() {
  ...
  return (
    <>
      <Head>
        <title>Domains – TempMail</title>
        <meta name="description" content={`Browse all ${domains.length}+ available domains for temporary email addresses`} />
      </Head>
                ...
                {filtered.map((d) => (
                  <div key={d.domain} className="grid grid-cols-12 gap-3 sm:gap-4 px-4 sm:px-5 py-3.5 items-center hover:bg-[#21262d]/40 transition-colors group">
                    <div className="col-span-7 sm:col-span-6 flex items-center gap-3 min-w-0">
                      ...
                      <span className="font-mono text-xs sm:text-sm text-white truncate">@{d.domain}</span>
                    </div>
